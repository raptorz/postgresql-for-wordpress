...
****/
	...
	function pg4wp_rewrite( $sql)
	{
		...
		if( 0 === strpos($sql, 'SELECT'))
		{
            ...
            // Fix SELECT @@SESSION.sql_mode error. by raptor
            elseif( false !== strpos($sql, '@@SESSION.sql_mode'))
            {
                $sql = "SELECT '' as \"@@SESSION.sql_mode\"";  //   STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION
            }
